...
"""
Julia set generator for f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3) where c = i/2

This explores the behavior of the function f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3) 
where c = i/2 is a fixed parameter and we iterate starting from various z values.
"""
...
def julia_iteration(z, c, max_iter):
    """
    Compute iterations of f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3) starting from z.
    ...
    for i in range(max_iter):
        ...
        # Apply the function f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3)
        z = 1/inner + inner
...
def compute_julia_set(x_min, x_max, y_min, y_max, width, height, max_iter, c):
    """
    Compute the Julia set for f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3) for a given region.
    ...
    for py in range(height):
        for px in range(width):
            ...
            julia_set[py, px] = julia_iteration(z, c, max_iter)
    
    return julia_set


def plot_julia_set(julia_set, x_min, x_max, y_min, y_max, c):
    """Plot the Julia set for f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3) with point marking."""
    ...
    ax.set_title(
        r'Julia set for f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3)' + '\n' +
...
def main():
    """Main function to compute and display the Julia set for f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3)."""
    # Set the parameter c = i/2
    c = 1j / 2
    ...
    print(f"Computing Julia set for f(z) = 1/(c (-3 z + z^3)) + c (-3 z + z^3)")
    ...
    julia_set = compute_julia_set(x_min, x_max, y_min, y_max, 
                                  width, height, max_iter, c)
    
    # Plot the result
    plot_julia_set(julia_set, x_min, x_max, y_min, y_max, c)
